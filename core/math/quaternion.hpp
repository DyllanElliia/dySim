...
#include "matALG.hpp"
namespace dym {
template <typename Type = Real> class Quaternion {
public:
  Type w;
  Vector<Type, 3> v;
...
template <typename Type>
_DYM_FORCE_INLINE_ Quaternion<Type>
lerp(const Quaternion<Type> &q0, const Quaternion<Type> &q1, const Real &t) {
  return (1 - t) * q0 + t * q1;
}
template <typename Type>
_DYM_FORCE_INLINE_ Quaternion<Type>
Nlerp(const Quaternion<Type> &q0, const Quaternion<Type> &q1, const Real &t) {
  Quaternion<Type> r = lerp(q0, q1, t);
  return r.normalize();
}
template <typename Type>
_DYM_FORCE_INLINE_ Quaternion<Type>
Slerp(const Quaternion<Type> &q0, const Quaternion<Type> &q1, const Real &t) {
