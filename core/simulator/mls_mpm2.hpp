 ...
 * @LastEditTime: 2022-02-21 15:46:25
 * @Description:
 */
#pragma once
#include "../dyMath.hpp"
...
u_int n_grid = 48, steps = 25;

u_int n_particles = std::pow(n_grid, dim) / (std::pow(2, dim - 1));
Real dx = 1 / Real(n_grid), inv_dx = Real(n_grid);
...
u_int addMaterial(const u_short &material_model, const Real &E = 0.1e4f,
                  const Real &nu = 0.2f, const Real &hardening = 10.f,
                  const Real &y_min = 1.f - 2.5e-2f,
                  const Real &y_max = 1.f + 8.5e-3f, const Real &opt = 0.f,
